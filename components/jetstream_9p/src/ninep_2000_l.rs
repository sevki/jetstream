...
pub trait NineP200L {
    ...
    async fn version(&mut self, tag: u16, version: &Tversion) -> io::Result<Rversion>;
    ...
    async fn attach(&mut self, tag: u16, attach: &Tattach) -> io::Result<Rattach>;

    /// The statfs message is used to retrieve file system information.
    async fn statfs(&mut self, tag: u16, statfs: &Tstatfs) -> io::Result<Rstatfs>;
    ...
    async fn lcreate(&mut self, tag: u16, lcreate: &Tlcreate) -> io::Result<Rlcreate>;

    /// The symlink message is used to create a symbolic link.
    async fn symlink(&mut self, tag: u16, symlink: &Tsymlink) -> io::Result<Rsymlink>;
    ...
    async fn readlink(&mut self, tag: u16, readlink: &Treadlink) -> io::Result<Rreadlink>;

    /// The getattr message is used to retrieve file attributes.
    async fn get_attr(&mut self, tag: u16, get_attr: &Tgetattr) -> io::Result<Rgetattr>;

    /// The setattr message is used to set file attributes.
    async fn set_attr(&mut self, tag: u16, set_attr: &Tsetattr) -> io::Result<()>;

    /// The xattrwalk message is used to traverse extended attributes.
    async fn xattr_walk(&mut self, tag: u16, xattr_walk: &Txattrwalk) -> io::Result<Rxattrwalk>;

    /// The xattrcreate message is used to create an extended attribute.
    async fn xattr_create(&mut self, tag: u16, xattr_create: &Txattrcreate) -> io::Result<()>;

    /// The readdir message is used to read a directory.
    async fn readdir(&mut self, tag: u16, readdir: &Treaddir) -> io::Result<Rreaddir>;
    ...
    async fn get_lock(&mut self, tag: u16, get_lock: &Tgetlock) -> io::Result<Rgetlock>;
    ...
    async fn rename_at(&mut self, tag: u16, rename_at: &Trenameat) -> io::Result<()>;

    /// The unlinkat message is used to remove a file.
    async fn unlink_at(&mut self, tag: u16, unlink_at: &Tunlinkat) -> io::Result<()>;
}
