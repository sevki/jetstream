use {
    futures::{Sink, Stream},
    jetstream_rpc::{Frame, Framer, Protocol},
    jetstream_wireformat::{wire_format_extensions::ConvertWireFormat, WireFormat},
    std::{
        io::{self, ErrorKind},
        marker::PhantomData,
        pin::Pin,
...
impl<P: Protocol> From<WebSocket<tungstenite::stream::MaybeTlsStream<std::net::TcpStream>>>
    for WebSocketTransport<P>
{
    fn from(value: WebSocket<tungstenite::stream::MaybeTlsStream<std::net::TcpStream>>) -> Self {
        Self(value, PhantomData)
    }
}

impl<P: Protocol> Sink<jetstream_rpc::Frame<P::Request>> for WebSocketTransport<P>
where
    Self: Unpin,
{
    type Error = io::Error;
