...
Therefore: any mechanism that names a subscription from **outside its own lane** — the datagram channel, a cancellation not carried on that lane, any control path a future revision adds — MUST use a **binding identifier that is unique for the lifetime of the session, never reused, and never zero**, not the tag and not an identifier recycled when a binding ends. A per-binding counter that only increases and starts at one satisfies this; recycling does not, because `r[jetstream.session.datagrams]` gives no instant at which the transport is known to have drained, so "no longer live" is not a condition an implementation can test.

Zero is reserved because a carrier of a binding identifier also has to express *no* binding identifier — a cancellation on the subscription's own lane needs none, since the tag suffices there — and reserving a value is cheaper on the wire than a separate flag. This document previously required only uniqueness and non-reuse while blessing "a per-binding counter that only increases", which is an allocation whose **first** identifier is zero: an off-lane cancellation naming that binding was then indistinguishable from a same-lane one, and the receiver would resolve `oldtag` against the wrong lane and cancel nothing, or cancel a different subscription holding that number. A reserved value that the same rule recommends colliding with is not reserved.
...
r[jetstream.subscription.termination.discriminant]
Where a terminator carries a typed payload and the terminator's message id is global, the payload MUST name the method it ends.

This is a contradiction between two rules that are each correct alone, and it appears only once a protocol has a *second* subscription method. `r[jetstream.subscription.compat]` puts the terminator in the global id space so that a streaming method costs no per-method id and `102 + 2 * index` is untouched — and a decoder is then handed one id and two possible terminal types. The tag would disambiguate them; a decoder does not see the tag, because decoding a message body is a function of the type byte alone. Naming the method in the payload costs one byte and is the only one of the three repairs that changes nothing else: the alternatives are to spend a per-method id on every terminator, which is what the compatibility rule exists to avoid, or to give decoding access to the tag, which changes the decoder's signature in every target language.

The identifier MUST be the message id of the request that opened the subscription, which is unique per method by construction and needs no separate registry.
...
Continuing to *accept* requests is not sufficient on its own. A dispatcher may satisfy the paragraph above by handing each request to a task — the shape `r[jetstream.rpc.ts.server-loop]` and `r[jetstream.webtransport.bidi.concurrent]` already describe, both of which hand each request frame to a spawned task — and `r[jetstream.lane.not-completion-order]` then permits those tasks to run in any order. A cancellation's task can therefore look for its target before the subscription's task has registered it, which is the "cancel a subscription that does not exist yet" failure of `r[jetstream.subscription.dispatch.issue-order]` reappearing at the *handler* rather than on the wire, and reached by a dispatcher that satisfies every rule as previously written.

A dispatcher MUST therefore make a streaming request's tag findable — registered, or recorded as pending — **before** it processes any later frame from that lane that could name it. This orders *registration* by the lane's frame order, which `r[jetstream.lane.delivery-order]` already fixes; it does not order *service*, and MUST NOT be implemented by serving one request before reading the next, which is what the first paragraph forbids. Registering synchronously as each frame is read satisfies both.
...
What establishment orders depends on when it completes, and a surface MUST say which of these it offers.

Where establishment completes when the request has been **queued on its lane**, it orders the subscription against later calls on *that lane* and nothing else. A subscription realised on its own lane, per `r[jetstream.subscription.realisation]`, is then unordered against a call on another lane — `r[jetstream.lane.no-cross-lane-order]` — and the surface MUST NOT present it as ordering one.

Where establishment completes on an **acknowledgement from the producer**, sent once the subscription is registered, it orders the subscription against every call the subscriber issues afterwards, on any lane. This is ordinary causality and not cross-lane frame ordering: the later call cannot be issued until the acknowledgement has arrived, and the acknowledgement cannot be sent until the subscription exists. A surface MAY offer this and SHOULD where the round trip is affordable, because it is the only form under which "subscribe, then act, then read" is correct without a cursor.

Neither form removes the need for `r[jetstream.subscription.resume]`. An acknowledged establishment orders what the *subscriber* does next; it says nothing about a third party writing to the same producer, and nothing about what happened before the subscription existed. An application that must not miss those says where to start and its producer replays from there — the practical content of the cursor requirement in `r[jetstream.subscription.surface.producer]`, and the reason a producer that only forwards live events loses messages under a realisation this document explicitly permits.
