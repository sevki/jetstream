...
fn stress_test_jetstream_client(c: &mut Criterion) {
    ...
    rt.block_on(async {
        ...
        c.bench_function("jetstream_client_stress", |b| {
            b.iter(|| {
                let response = rt.block_on(handle.rpc("stress".to_string()));
...
fn stress_test_jetstream_server(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();
    rt.block_on(async {
...
criterion_group!(
    benches,
    stress_test_jetstream_client,
    stress_test_jetstream_server
);
